...
export class OrganizationsComponent implements OnInit {
  ...
  addNewDept() {
    (this.organizationForm.controls.dept as FormArray).push(
      this.addDeptFormGroup()
    );
  }
  ...
  calcManagerCost(m: FormGroup) {
    ...
    const subManagerCost = (m.get('manager') as FormArray).controls.reduce(
      (total, el: FormGroup) => {
  ...
  calcDeptCost(d) {
    return (d.get('manager') as FormArray).controls.reduce(
      (total, el: FormGroup) => {
